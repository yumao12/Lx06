    ...
    public class Comment
    {
        ...
        public Model.Comment GetModel(int id) {
            using (IDbConnection cn = new MySqlConnection(cns)) {
                string sql = "select * from Coment where commentID=@id";
        ...
        public IEnumerable<Model.CommentNo> GetPage(Model.Page page) {
            using (IDbConnection cn = new MySqlConnection(cns)) {
                ...
                sql += "select * from a where num between (@pageIn-1)*@pageSize+1 and @pageIndex*@pageSize;";
        ...
        public int Update(Model.Comment comment)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update Comment set CommentContent=@CommentContent,CommentTime=@CommentCotent where commentID=@commentID";
                return cn.ExecuteScalar<int>(sql, comment);
            }
        }
        public int Delete(int id)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                return cn.ExecuteScalar<int>(sql, new { id=id});
            }
        }
    }
}
